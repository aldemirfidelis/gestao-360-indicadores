...
export function SnapshotsTab() {
  ...
  const create = useMutation({ mutationFn: () => api('/admin/portal/snapshots', { method: 'POST', json: { label } }), onSuccess: () => { toast.success('Retrato criado.'); setLabel(''); inv(); }, onError: (e: ApiError) => toast.error(e.message) });
  const del = useMutation({ mutationFn: (id: string) => api(`/admin/portal/snapshots/${id}`, { method: 'DELETE' }), onSuccess: inv });
  const restoreMut = useMutation({ mutationFn: () => api(`/admin/portal/snapshots/${restore!.snap.id}/restore`, { method: 'POST', json: { confirmationPhrase: restore!.phrase } }), onSuccess: () => { toast.success('Configuração restaurada (retrato preventivo criado).'); setRestore(null); inv(); qc.invalidateQueries({ queryKey: ['portal'] }); }, onError: (e: ApiError) => toast.error(e.message) });
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="Criar retrato" description="Captura o estado atual (módulos, páginas, funcionalidades, opções e menus).">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Button onClick={() => create.mutate()} disabled={create.isPending}><Camera className="mr-2 h-4 w-4" />Criar retrato</Button>
        </div>
      </SectionCard>
      <SectionCard title="Histórico de configuração" description="Comparar, restaurar (com confirmação e retrato preventivo) ou excluir." contentClassName="p-0">
        {list.isLoading && <LoadingState label="Lendo retratos..." />}
        {(list.data ?? []).length === 0 ? <div className="p-4 text-sm text-muted-foreground">Nenhum retrato. Retratos automáticos também são criados antes de restaurações.</div> : (
        ...
        <Dialog open onOpenChange={(o) => !o && setRestore(null)}>
          <DialogContent>
            ...
            <div className="space-y-3 text-sm">
              <p>Restaurar <strong>{restore.snap.label}</strong> reverte status de módulos/páginas/funcionalidades/opções/menus para o estado do retrato. Um retrato preventivo do estado atual será criado.</p>
      ...
      {diff && (
        <Dialog open onOpenChange={(o) => !o && setDiff(null)}>
          <DialogContent className="max-w-xl">
            ...
            {(!diff.data || diff.data.changes.length === 0) ? <p className="text-sm text-muted-foreground">Nenhuma diferença: o estado atual é igual ao do retrato.</p> : (
              <div className="max-h-[50vh] overflow-auto">
                <table className="table-modern"><thead><tr><th className="text-left">Tipo</th><th className="text-left">Código</th><th className="text-left">Retrato</th><th className="text-left">Atual</th></tr></thead>
