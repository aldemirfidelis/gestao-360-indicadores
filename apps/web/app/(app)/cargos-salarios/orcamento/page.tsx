...
export default function OrcamentoPessoalPage() {
  ...
  const createBudget = useMutation({
    ...
    onSuccess: () => {
      toast.success('Orcamento cadastrado');
    ...
    onError: (error: any) => toast.error(error?.message ?? 'Falha ao cadastrar orcamento'),
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Orcamento de Pessoal"
        description="Planejamento de headcount, folha, beneficios e encargos por periodo, area e centro de custo."
        breadcrumbs={[{ label: 'Inicio', href: '/' }, { label: 'Cargos e Salarios', href: '/cargos-salarios' }, { label: 'Orcamento de Pessoal' }]}
      />
      ...
      <SectionCard title={`Orcamentos cadastrados - folha planejada ${formatMoney(totalPayroll)}`} className="mt-4">
