...
export default function TerminationsPage() {
  ...
  const createTermination = useMutation({
    mutationFn: (json: any) => api('/payroll/terminations', { method: 'POST', json }),
    onSuccess: (data: any) => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // Calcular verbas simuladas básicas no frontend para exibição inicial e enviar ao backend
    const emp = employeeMap.get(selectedEmpId);
    const hiringDate = emp ? new Date(emp.hiringDate) : new Date();
    const termDate = new Date(form.terminationDate);
    
    // Contar avos aproximados de 13º e Férias
    const months = (termDate.getFullYear() - hiringDate.getFullYear()) * 12 + (termDate.getMonth() - hiringDate.getMonth());
    const vacationAvos = Math.max(1, months % 12);
    const thirteenthAvos = termDate.getMonth() + 1;

    // Resultados fictícios que o backend vai recalcular/gravar
    const resultsJson = {
      vacationAvos,
      thirteenthAvos,
      grossValue: 450000, // R$ 4.500,00 simulado
      deductionsValue: 45000, // R$ 450,00
      netValue: 405000, // R$ 4.050,00
    };
  ...
  return (
    <div className="space-y-6">
        ...
        <DialogContent className="max-w-[480px]">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="p-3 bg-muted/40 border border-muted/80 rounded-md text-[11px] text-muted-foreground flex gap-1.5">
              ...
              <div>
                O cálculo de rescisão simula saldo de salário proporcional, avos de 13º proporcional, avos de férias proporcionais com acréscimo de 1/3 e aviso prévio indenizado proporcional ao tempo de serviço (Art. 487 CLT).
              </div>
            </div>
      ...
      {/* DIALOG: Resultado do Cálculo da Rescisão */}
      <Dialog open={showResult} onOpenChange={setShowResult}>
        <DialogContent className="max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Simulador de Rescisão — Verbas Simuladas</DialogTitle>
          </DialogHeader>
          {simulationResult && (
            <div className="space-y-4">
              ...
              <div className="border rounded-md divide-y text-sm">
                <div className="p-3 flex justify-between">
                  <span>Projeção de 13º Salário Proporcional</span>
                  <span className="font-semibold">{simulationResult.resultsJson?.thirteenthAvos}/12 avos</span>
                </div>
                <div className="p-3 flex justify-between">
                  <span>Projeção de Férias Proporcionais</span>
                  <span className="font-semibold">{simulationResult.resultsJson?.vacationAvos}/12 avos</span>
                </div>
                <div className="p-3 flex justify-between bg-muted/20 font-bold">
                  <span>Estimativa de Verbas Brutas</span>
                  <span>{formatCurrency(simulationResult.resultsJson?.grossValue)}</span>
                </div>
                <div className="p-3 flex justify-between text-muted-foreground">
                  <span>Deduções de Encargos Proporcionais</span>
                  <span>- {formatCurrency(simulationResult.resultsJson?.deductionsValue)}</span>
                </div>
                <div className="p-3 flex justify-between bg-rose-500/5 font-extrabold text-rose-600 dark:text-rose-400 text-lg border-t">
                  ...
                  <span>{formatCurrency(simulationResult.resultsJson?.netValue)}</span>
                </div>
              </div>

              <DialogFooter className="pt-2">
                <Button onClick={() => setShowResult(false)}>Fechar Simulação</Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
